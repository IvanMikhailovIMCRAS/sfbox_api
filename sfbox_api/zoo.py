...
def barbwire(p: int, n: int, m: int, q: int, root: int, 
    ...
    if p == 1:
        comp = f"{root_block}{side_block}(A){m-1}(G)1"
    else:
        comp = f"{root_block}{side_block}((A){m}{side_block}){p-1}(A){m-1}(G)1"

    # lattice parameters
    lat = Lat.model_validate({"n_layers": n_layers, "geometry": geometry})
    mons = [
        Mon.model_validate({"name": "X", "freedom": "pinned", "pinned_range": "1"}),
        Mon.model_validate({"name": "A", "freedom": "free"}),
        Mon.model_validate({"name": "G", "freedom": "free"}),
    ...
    # collecting frame for calculation
    frame = Frame(lat, Sys(), mols, mons, chi_list=chi_list)
    frame.run()
    return frame
